...
def delete_passenger(driver, passenger):
    time.sleep(2)
    ...
    for row in rows:
        if passenger["email"] in row.text:
            try:
                ...
                print(f"🗑️ Clicked delete for {passenger['email']}")
            except Exception:
                raise Exception("❌ Delete button not found in the passenger's row.")
            break
    else:
        raise Exception(f"❌ Could not find passenger with email for deletion: {passenger['email']}")
    ...
    try:
        ...
        continue_btn.click()
        print(f"✅ Successfully deleted passenger {passenger['email']}")

    except Exception as e:
        print(f"❌ Delete process failed for {passenger['email']}: {e}")
        raise