...
def delete_b2b(driver, updated_b2b):
    ...
    for row in rows:
        if updated_b2b["email"] in row.text:
            try:
                ...
                print(f"🗑️ Clicked delete for {updated_b2b['email']}")
            except Exception:
                raise Exception("❌ Delete button not found in the b2b user's row.")
            break
    else:
        raise Exception(f"❌ Could not find b2b user with email for deletion: {updated_b2b['email']}")
    ...
    try:
        ...
        continue_btn.click()
        print(f"✅ Successfully deleted b2b user {updated_b2b['email']}")

    except Exception as e:
        print(f"❌ Delete process failed for {updated_b2b['email']}: {e}")
        raise