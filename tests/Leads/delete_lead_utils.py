...
def delete_lead(driver, lead):
    ...
    for row in rows:
        if lead["name"] in row.text:
            try:
                ...
                print(f"🗑️ Clicked delete for {lead['name']}")
            except Exception:
                raise Exception("❌ Delete button not found in the row.")
            break
    else:
        raise Exception(f"❌ Could not find lead for deletion: {lead['name']}")
    ...
    try:
        ...
        continue_btn.click()
        print(f"✅ Successfully deleted lead {lead['name']}")

    except Exception as e:
        print(f"❌ Delete process failed for {lead['name']}: {e}")
        raise