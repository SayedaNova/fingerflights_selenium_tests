...
def delete_user(driver, user):
    ...
    for row in rows:
        if user["email"] in row.text:
            try:
                ...
                print(f"🗑️ Clicked delete for {user['email']}")
            except Exception:
                raise Exception("❌ Delete button not found in the user's row.")
            break
    else:
        raise Exception(f"❌ Could not find user with email for deletion: {user['email']}")
    ...
    try:
        ...
        print(f"❌ Delete process failed for {user['email']}: {e}")
        raise